...
public class SimpleAlloyKilnRecipe implements Recipe<SimpleContainer> {
    ...
    public SimpleAlloyKilnRecipe(ResourceLocation id, ItemStack output, NonNullList<Ingredient> recipeItems, int heatReq) {
        this.id = id;
        this.output = output;
    ...
    public int getHeatReq() { return heatReq; }
    ...
    public static class Serializer implements RecipeSerializer<SimpleAlloyKilnRecipe> {
        ...
        public SimpleAlloyKilnRecipe fromJson(ResourceLocation id, JsonObject json) {
            ...
            int heatReq = GsonHelper.getAsInt(json, "heat_req");
            for (int i = 0; i < inputs.size(); i++) {
                inputs.set(i, Ingredient.fromJson(ingredients.get(i)));
            }

            return new SimpleAlloyKilnRecipe(id, output, inputs, heatReq);
        }

        @Override
        public SimpleAlloyKilnRecipe fromNetwork(ResourceLocation id, FriendlyByteBuf buf) {
            NonNullList<Ingredient> inputs = NonNullList.withSize(buf.readInt(), Ingredient.EMPTY);
            int heatReq = buf.readInt();
            for (int i = 0; i < inputs.size(); i++) {
